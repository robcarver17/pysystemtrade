...
def sort_keywords_by_underscore_length(other_arg_keys):
    ...
    counted_keys = dict([(key_value, count_leading_underscores_in_string(key_value)) for key_value in other_arg_keys])
    count_values = list(counted_keys.values())
    max_key = max(count_values)

    sorted_keywords = []
    for key_len in range(max_key+1):
        keys_with_key_value = [key for key, key_value in counted_keys.items() if key_value == key_len]
...
def count_leading_underscores_in_string(process_string):
    ...
    if len(process_string)==0:
        raise Exception("Can't pass a parameter name consisting only of underscores or a zero length string")
...
def strip_underscores_from_dict_keys(arg_dict):
    ...
    new_dict = dict([(strip_leading_underscores_from_str(old_key), dict_value)
...
def force_args_to_same_length(data_args_input, data, pad_with={}):
    ...
    while len(padded_data_args) < len(data):
        if len(data_args) == 0:
            # We have exhausted all the available data, need to pad with empty dicts
