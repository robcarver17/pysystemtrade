...
class Test(ut.TestCase):
    ...
    def test_robust_vol_calc_min_period(self):
        prices=pd_readcsv_frompackage("syscore", "pricetestdata_min_period.csv", ["tests"])
        returns=prices.diff()
    ...
    def test_robust_vol_calc_min_value(self):
        prices=pd_readcsv_frompackage("syscore", "pricetestdata_zero_vol.csv", ["tests"])
        returns=prices.diff()
        vol=robust_vol_calc(returns, vol_abs_min=0.01)
        self.assertEqual(vol.iloc[-1,0], 0.01)

    def test_robust_vol_calc_floor(self):
        prices=pd_readcsv_frompackage("syscore", "pricetestdata_vol_floor.csv", ["tests"])
        returns=prices.diff()
