...
class stackHandlerCreateBalanceTrades(stackHandlerForCompletions):
    ...
    def create_balance_trade(self, broker_order):
        ...
        instrument_order = create_balance_instrument_order_from_contract_order(contract_order)

        log.msg("Putting balancing trades on stacks")

        result, instrument_order_id, contract_order_id, broker_order_id = self.put_balance_trades_on_stack(instrument_order, contract_order, broker_order)
    ...
    def put_balance_trades_on_stack(self, instrument_order, contract_order, broker_order):
        ...
        if type(instrument_order_id) is not int:
            log.error("Couldn't add balancing instrument trade error condition %s" % str(instrument_order_id))
        ...
        if type(contract_order_id) is not int:
            log.error("Couldn't add balancing contract trade error condition %s" % str(contract_order_id))
            return failure, instrument_order_id, missing_order, missing_order

        result = self.instrument_stack.add_children_to_order(instrument_order_id, [contract_order_id])
        ...
        if type(broker_order_id) is not int:
            log.error("Couldn't add balancing broker trade error condition %s" % str(broker_order_id))
            return failure, instrument_order_id, contract_order_id, missing_order

        result = self.contract_stack.add_children_to_order(contract_order_id, [broker_order_id])
        if result is not success:
    ...
    def rollback_balance_trades(self, instrument_order_id, contract_order_id, broker_order_id):
...
def create_balance_contract_order_from_broker_order(broker_order):
    contract_order = contractOrder(broker_order.strategy_name, broker_order.instrument_code, broker_order.contract_id,
                  ...
                  algo_to_use=broker_order.algo_used, filled_price=broker_order.filled_price,
                  fill_datetime=broker_order.fill_datetime, manual_fill=True, manual_trade=True,
                  active=False)

    return contract_order

def create_balance_instrument_order_from_contract_order(contract_order):
    instrument_order = instrumentOrder(contract_order.strategy_name, contract_order.instrument_code,
                                       contract_order.trade[0], fill=contract_order.fill[0],
                                       filled_price=contract_order.filled_price,
                                       fill_datetime=contract_order.fill_datetime,
                                       manual_trade=True, active=False
                                       )
