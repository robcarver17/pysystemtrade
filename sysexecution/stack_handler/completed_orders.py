...
class stackHandlerForCompletions(stackHandlerCore):
    ...
    def split_up_spread_orders(self, instrument_order_id):
        ...
        :return: success
        """
        original_instrument_order = self.instrument_stack.get_order_with_id_from_stack(instrument_order_id)
        ...
        for contract_order_id in list_of_contract_order_id:
            new_contract_order_ids_for_this_contract = self.split_contract_order_id(contract_order_id)
            new_contract_order_ids = new_contract_order_ids + new_contract_order_ids_for_this_contract

        # add the new contract order ids as children for the instrument order
        for contract_order_id in new_contract_order_ids:
            self.instrument_stack.add_another_child_to_order(instrument_order_id, contract_order_id)
    ...
    def split_contract_order_id(self, contract_order_id):
        ...
        for broker_order_id in existing_broker_ids:
            new_order_ids_for_this_broker_id = self.split_broker_order_id(broker_order_id)

            # add the new broker order ids to the list
            new_broker_order_ids = new_broker_order_ids + new_order_ids_for_this_broker_id

        # add the new broker order ids as children for the original contract order (keep the original children)
        for new_broker_order_id in new_broker_order_ids:
            self.contract_stack.add_another_child_to_order(contract_order_id, new_broker_order_id)

        new_contract_order_ids_for_this_contract = self.split_contract_order_id_once_broker_id_split(contract_order_id)
    ...
    def split_contract_order_id_once_broker_id_split(self, contract_order_id):
        ...
        if original_contract_order.calendar_spread_order:
            ...
            new_contract_order_ids_for_this_contract = \
                self.contract_stack.put_list_of_orders_on_stack(component_contract_orders)
        else:
            new_contract_order_ids_for_this_contract = []
    ...
    def split_broker_order_id(self, broker_order_id):
        ...
        original_broker_order = self.broker_stack.get_order_with_id_from_stack(broker_order_id)
        ...
        if original_broker_order.calendar_spread_order:
            ...
            # set the original broker order to zero fill and deactivate so we don't try and fill it
