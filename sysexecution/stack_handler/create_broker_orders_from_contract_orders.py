...
class stackHandlerCreateBrokerOrders(stackHandlerCore):
    ...
    def create_broker_order_for_contract_order(self, contract_order_id, check_if_open=True):
        ...
        contract_order = self.preprocess_contract_order(original_contract_order, check_if_open=check_if_open)
        ...
        broker_order_with_controls = self.add_trade_to_database(broker_order_with_controls)
        broker_order_with_controls = algo_instance.manage_trade(broker_order_with_controls)

        result = self.post_trade_processing(broker_order_with_controls)

        return result

    def preprocess_contract_order(self, original_contract_order, check_if_open = True):
    ...
    def size_contract_order(self, original_contract_order):
        ...
        contract_order_after_trade_limits = self.what_contract_trade_is_possible(remaining_contract_order)

        contract_order = self.liquidity_size_contract_order(contract_order_after_trade_limits)
    ...
    def liquidity_size_contract_order(self, contract_order_after_trade_limits):
        ...
        liquid_qty = data_broker.get_largest_offside_liquid_size_for_contract_order_by_leg(contract_order_after_trade_limits)
        ...
        if liquid_qty.equals_zero():
            ...
            log.msg("Can't do any of size %s so not trading at all" % str(contract_order_after_trade_limits.trade))

            return missing_order

        contract_order = contract_order_after_trade_limits.replace_trade_only_use_for_unsubmitted_trades(liquid_qty)
    ...
    def resolve_algo(self, contract_order):
        ...
        ## Very important to avoid multiple broker orders being issued from the same contract order
        self.contract_stack.add_controlling_algo_ref(contract_order.order_id, algo_to_use_str)
    ...
    def apply_fills_to_database(self, broker_order):
        ...
        self.broker_stack.change_fill_quantity_for_order(broker_order_id,
                                                         broker_order.fill,
