...
def post_trade_processing(data, broker_order_with_controls):
    ...
    ## This order will now hopefully contain all fills so we set trades==fills so the order is treated as completed
...
def set_limit_price(data, broker_order_with_controls, new_limit_price):
    log = broker_order_with_controls.order.log_with_attributes(data.log)
    data_broker = dataBroker(data)
    can_be_modified = data_broker.check_order_can_be_modified_given_control_object(broker_order_with_controls)
    if can_be_modified:
        broker_order_with_controls = \
            data_broker.modify_limit_price_given_control_object(broker_order_with_controls, new_limit_price)
...
def check_current_limit_price_at_inside_spread(broker_order_with_controls):
    # When we are aggressive we want to remain on the correct side of the spread

    correct_limit_price_on_order = broker_order_with_controls.check_limit_price_consistent()
