"""
For a given list of futures contracts defined by Barchart start dates:

read price data from barchart, and then write to artic
Write list of futures contracts to mongodb database
"""

from sysdata.barchart.barchart_futures import barchartFuturesConfiguration, barchartFuturesContractPriceData
...
def get_first_contract_date_from_barchart(instrument_code):
    config = barchartFuturesConfiguration()
...
def create_list_of_contracts(instrument_code):
    ...
    first_contract_date = get_first_contract_date_from_barchart(instrument_code)
...
def get_and_write_prices_for_contract_list_from_barchart_to_arctic(list_of_contracts):
    barchart_prices_data = barchartFuturesContractPriceData()
    ...
    for contract_object in list_of_contracts:
        ...
        barchart_price = barchart_prices_data.get_prices_for_contract_object(contract_object)

        if barchart_price.empty:
            print("Problem reading price data this contract - skipping")
        else:
            print("Read ok, trying to write to arctic")
            try:
                arctic_prices_data.write_prices_for_contract_object(contract_object, barchart_price)
            except:
                raise Exception("Some kind of issue with arctic - stopping so you can fix it")


if __name__ == '__main__':
    instrument_code = "SILVER"
    ...
    get_and_write_prices_for_contract_list_from_barchart_to_arctic(list_of_contracts)
