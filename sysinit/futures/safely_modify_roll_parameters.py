...
def safely_modify_roll_parameters(data: dataBlob):
    ...
    instrument_code = get_valid_instrument_code_from_user(data,
                                                          source=INSTRUMENT_CODE_SOURCE_CONFIG)
    new_roll_parameters = modified_roll_parameters(data,
                                                   instrument_code=instrument_code)

    output_path_for_roll_calendar = input("Path for writing roll calendar; must be absolute with leading \ or / eg /home/rob/pysystemtrade/data/futures/roll_calendars_csv/")
    ...
    ans = true_if_answer_is_yes('Inspect roll calendar, and if required manually hack or change roll parameters. Happy to continue?')
    if not ans:
        print("Doing nothing")
        #return None
    ...
    do_the_plots = true_if_answer_is_yes('Display diagnostic plots? Answer NO on headless server')
    if do_the_plots:
        prices = pd.concat([new_multiple_prices.PRICE,
                           existing_multiple_prices.PRICE], axis=1)
        prices.columns = ['New', 'Existing']

        prices.plot(title="Prices of current contract")

        carry_price = pd.concat([new_multiple_prices.CARRY,
                           existing_multiple_prices.CARRY], axis=1)
        ...
        net_carry_compare = pd.concat([net_carry_new,
                                      net_carry_existing], axis=1)
        net_carry_compare.columns =  ['New', 'Existing']
        net_carry_compare.plot(title="Raw carry difference")

        adj_compare = pd.concat([
                                 existing_adj_prices,new_adjusted_prices],
                                axis=1)
    ...
    sure = true_if_answer_is_yes("Happy to continue? Saying YES will overwrite existing data!")
    if not sure:
    ...
    data_contracts.update_roll_parameters(instrument_code=instrument_code,
                                          roll_parameters=new_roll_parameters
                                          )
    print("Updated roll parameters in database. Use interactive controls to copy to .csv")

    ## Overwrite multiple prices
    update_prices = updatePrices(data)
    update_prices.add_multiple_prices(instrument_code=instrument_code,
    ...
    update_prices.add_adjusted_prices(instrument_code=instrument_code,
...
def modified_roll_parameters(data: dataBlob,
    ...
    while unhappy:
        ...
        approx_expiry_offset = get_and_convert('Approximate expiry day in month', type_expected=int,
                                       default_value=roll_parameters.approx_expiry_offset)

        try:
            new_roll_parameters = rollParameters(hold_rollcycle=hold_rollcycle,
