...
class accountTradingRules(accountForecast):
    ...
    def pandl_for_trading_rule_weighted(self, rule_variation_name: str,
                                        delayfill: bool=True) \
                                        -> accountCurveGroup:
    ...
    @diagnostic(not_pickable=True)
    def pandl_for_all_trading_rules(self,
                                    delayfill: bool = True)\
        ...
        dict_of_pandl_by_rule = dict(
            [(
                rule,
                self.pandl_for_trading_rule_weighted(rule, delayfill=delayfill)
            )
        ...
        account_curve = nestedAccountCurveGroup(dict_of_pandl_by_rule,
                                                capital = capital,
                                                weighted = True)
    ...
    @diagnostic(not_pickable=True)
    def pandl_for_all_trading_rules_unweighted(self,
                                               delayfill: bool = True):
        ...
        dict_of_pandl_by_rule = dict(
            [(
                rule,
                self.pandl_for_trading_rule(rule, delayfill=delayfill)
            )
                for rule in list_of_rules
            ]
        )
    ...
    @diagnostic(not_pickable=True)
    def pandl_for_instrument_rules(self, instrument_code: str,
