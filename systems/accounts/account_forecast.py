...
class accountForecast(accountCosts):
    ...
    def pandl_for_instrument_forecast_weighted_within_trading_rule(self,
                                                                   instrument_code: str,
                                                                   rule_variation_name: str,
                                                                   delayfill: bool = True
                                                                   ) -> accountCurve:

        pandl_for_instrument_forecast = self.pandl_for_instrument_forecast(instrument_code,
                                                                           rule_variation_name,
                                                                           delayfill=delayfill)
    ...
    @diagnostic(not_pickable=True)
    def pandl_for_instrument_forecast_weighted(self, instrument_code: str,
            rule_variation_name: str,
            delayfill: bool=True
            ) -> accountCurve:
        ...
        weight = self._normalised_weight_for_forecast_and_instrument(instrument_code,
                                                                     rule_variation_name=rule_variation_name)
    ...
    @diagnostic()
    def _normalised_weight_for_forecast_and_instrument_within_trading_rule(self,
                                                                           instrument_code: str,
                                                                           rule_variation_name: str,
                                                                           ) -> pd.Series:
        weight = self._unnormalised_weight_for_forecast_and_instrument(instrument_code=instrument_code,
                                                                       rule_variation_name=rule_variation_name)

        total_weight_for_rule = self._total_unnormalised_weight_for_trading_rule(rule_variation_name)
    ...
    @diagnostic()
    def _total_unnormalised_weight_for_trading_rule(self, rule_variation_name: str) -> pd.Series:
        list_of_instruments = self.get_instrument_list()

        list_of_weights = [self._unnormalised_weight_for_forecast_and_instrument(instrument_code,
                                                                                 rule_variation_name=rule_variation_name)
    ...
    @diagnostic()
    def _normalised_weight_for_forecast_and_instrument(self, instrument_code: str,
                                                       rule_variation_name: str) -> pd.Series:

        weight = self._unnormalised_weight_for_forecast_and_instrument(instrument_code=instrument_code,
                                                                       rule_variation_name=rule_variation_name)
        total_weight = self._total_unnormalised_weight_across_all_instruments_and_forecasts()
    ...
    @diagnostic()
    def _total_unnormalised_weight_across_all_instruments_and_forecasts(self) -> pd.Series:
        list_of_instruments = self.get_instrument_list()
        list_of_weights = [self._total_unnormalised_weight_for_instrument(instrument_code)
                           for instrument_code in list_of_instruments]
    ...
    @diagnostic()
    def _total_unnormalised_weight_for_instrument(self, instrument_code: str) -> pd.Series:
    ...
    @diagnostic()
    def _unnormalised_weight_for_forecast_and_instrument(self, instrument_code: str,
                                                         rule_variation_name: str) -> pd.Series:
        ...
        forecast_weight = self.forecast_weight(instrument_code=instrument_code,
                                               rule_variation_name=rule_variation_name)
...
def pandl_for_instrument_forecast(forecast: pd.Series,
                                  ...
                                  SR_cost = 0.0,
                                  delayfill = True,
                                  value_per_point = ARBITRARY_VALUE_OF_PRICE_POINT) -> accountCurve:
...
def _get_average_notional_position(daily_returns_volatility: pd.Series,
                                   capital: float = ARBITRARY_FORECAST_CAPITAL,
                                   risk_target: float = ARBITRARY_FORECAST_ANNUAL_RISK_TARGET_PERCENTAGE,
                                   value_per_point = ARBITRARY_VALUE_OF_PRICE_POINT) -> pd.Series:
