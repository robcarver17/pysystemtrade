...
for Xfactor in [16 * 0.025]:
    ...
    config = Config(
        dict(
            trading_rules=dict(simple_mav=random_rule),
            Xfactor=Xfactor,
...
for fastmom in [1, 2, 4, 8, 16, 32, 64, 128, 256]:
    ...
    system.set_logging_level("on")
    all_positions = [
        system.rules.get_raw_forecast(instrument, "rule0")
        for instrument in system.get_instrument_list()
    ]
...
fastmom = 16
simple_mav_rule = TradingRule(
    dict(function=simple_mav, other_args=dict(long=4 * fastmom, short=fastmom))
)
...
# no error bars
SR_by_instrument_list = [
    system.accounts.pandl_for_subsystem(code).gross.sharpe()
    for code in system.get_instrument_list()
]
SR_by_instrument = pd.Series(
    SR_by_instrument_list,
...
SR_by_instrument_list_bars = [
    SR_error_bars(system.accounts.pandl_for_subsystem(code).gross)
    for code in system.get_instrument_list()
]
SR_by_instrument_bars = pd.DataFrame(
    SR_by_instrument_list_bars,
    system.get_instrument_list(),
...
filename = (
    "/home/rob/workspace3/pysystemtrade/systems/provided/simplersystem/costsvssize.csv"
)
costs_vs_size = pd.read_csv(filename)
costs_vs_size["trading_cost"] = (
    costs_vs_size.Execution_cost_vol * trades_each_year_count
    + costs_vs_size.Holding_cost_vol
)
...
for product, marker in zip(product_types, marker_types):
    ...
    print(
        "Product %s cost %.2f min_size %.2f"
        % (
            product,
            cs_df_this_product.trading_cost.median(),
            cs_df_this_product.Min_size.median(),
        )
    )
...
together[pd.datetime(2018, 6, 18): pd.datetime(2018, 11, 8)].ffill().plot(
    lw=4, style=["-", "--", "-."]
)
frame = plt.gca()
...
config = Config(
    dict(
        trading_rules=trading_rule_dict,
        Xfactor=8,
        percentage_vol_target=16.0))
data = csvFuturesSimData()
...
all_average_SRs = [
    get_SR_bars_for_rule(
        rulename,
        system) for rulename in ordered_names]
SR_by_rule_bars = pd.DataFrame(
    all_average_SRs, ordered_names, columns=["lower", "mean", "upper"]
)
...
for instrument in ["GOLD", "CORN", "EUROSTX", "GBP"]:
    ...
    together[pd.datetime(2018, 6, 1): pd.datetime(2018, 11, 8)].plot(
        lw=4, style=["-", "--", "-."]
    )
    plt.rcParams.update({"font.size": 24})
...
config = Config(
    dict(
        ...
        Xfactor=8,
        percentage_vol_target=16.0,
        weights=dict(GOLD=0.25, CORN=0.25, EUROSTX=0.25, GBP=0.25),
    )
)
