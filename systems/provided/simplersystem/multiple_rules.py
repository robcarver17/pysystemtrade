...
simple_mav_rules=dict([("mac_%d_%d" %(fast_length, fast_length*4),
    TradingRule(dict(function = simple_mav, other_args=dict(long=fast_length*4, short=fast_length))))
    for fast_length in [2,4,8,16,32,64]])

simple_mav_rules_ordered_keys = ["mac_%d_%d" %(fast_length, fast_length*4)
    for fast_length in [2,4,8,16,32,64]]
...
breakouts = dict([("breakout_%d" % (lookback),
    TradingRule(dict(function = simple_breakout, other_args = dict(lookback = lookback))))
    for lookback in [10, 20, 40, 80, 160, 320]])

breakouts_ordered_keys = ["breakout_%d" % (smooth)
    for smooth in [10, 20, 40, 80, 160, 320]]


carry_rules = dict([("carry_%d" % (smooth_days),
    TradingRule(dict(function = carry2, other_args = dict(smooth_days = smooth_days))))
    for smooth_days in [10, 30, 60, 125]])

carry_ordered_keys = ["carry_%d" % (smooth_days)
    for smooth_days in [10, 30, 60, 125]]


relative_carry_rule = dict(relative_carry = TradingRule(dict(function= relative_carry,
...
#all_rules = {**simple_mav_rules, **ewmac_rules, **breakouts, **carry_rules, **relative_carry_rule}
...
for (ordered_names, rule_set_name, rule_set) in zip(
    ...
    config= Config(dict(trading_rules = rule_set, stop_fraction=.5,
    ...
    system = System([
            ...
            ForecastCombine(), ForecastScaleCap(), Rules_with_binary(rule_set)
        ], data, config)
...
config = Config(dict(trading_rules=all_rules, stop_fraction=.5,
                     percentage_vol_target=12.0,
                     use_instrument_div_mult_estimates=True,
                     ))
data = csvFuturesSimData()
system = System([
    Account(), Portfolios(), PositionSizeWithStopLoss(), FuturesRawData(),
    ForecastCombine(), ForecastScaleCap(), Rules_with_binary(all_rules)
], data, config)
system.set_logging_level("on")

system.config.forecast_weight_estimate['ceiling_cost_SR']=9999

ordered_rule_names = simple_mav_rules_ordered_keys+  breakouts_ordered_keys + ['carry']

all_average_SRs = [get_SR_bars_for_rule(rulename, system) for rulename in ordered_rule_names]
SR_by_rule_bars = pd.DataFrame(all_average_SRs, ordered_rule_names, columns=["lower", "mean", "upper"])
SR_by_rule_bars.transpose().plot(kind="box", rot=90)
plt.rcParams.update({'font.size': 24})
plt.gcf().subplots_adjust(bottom=0.35)
...
for rule_count in range(1,rule_length):
    ...
    def pick_rules(all_rule_names, rule_count, rule_length):
        ...
        pick_indices = [possible_indices.pop(int(random.uniform(0, len(possible_indices)))) for notUsed in
                        range(rule_count)]

        rule_name_list = [all_rule_names[rule_index] for rule_index in pick_indices]

        return rule_name_list

    possible_perms = [pick_rules(ordered_rule_names, rule_count, rule_length) for notUsed in
                  range(MAX_PERMS)]

    curves = [results_stack_given_a_rule_subset(all_rule_pandl_frame, rule_name_list) for rule_name_list in possible_perms]
    curve_stack = pd.concat(curves, axis=0)
    sr_bars = SR_error_bars_from_stacked(curve_stack)

    all_results.append(sr_bars)

SR_by_rule_bars = pd.DataFrame(all_results, list(range(1, rule_length)), columns=["lower", "mean", "upper"])
SR_by_rule_bars.transpose().plot(kind="box", rot=90)
plt.rcParams.update({'font.size': 24})
plt.gcf().subplots_adjust(bottom=0.35)
...
## plot some crude
price = data.get_raw_price("CRUDE_W")[pd.datetime(2010,1,1):pd.datetime(2016,1,1)]
price.plot()
plt.rcParams.update({'font.size': 24})
...
stacked_stuff.set_axis(["price", "Max", "Min", "Avg"], axis='columns', inplace=True)
stacked_stuff.plot()
plt.rcParams.update({'font.size': 24})
