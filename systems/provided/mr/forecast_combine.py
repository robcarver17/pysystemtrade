...
class MrForecastCombine(ForecastCombine):
    ...
    def get_combined_forecast(self, instrument_code: str) -> pd.Series:
        ...
        forecast_after_filter = apply_forecast_filter(forecast_before_filter, conditioning_forecast)
    ...
    def conditioning_forecast(self, instrument_code) -> pd.Series:
        ...
        return self._get_capped_individual_forecast(instrument_code=instrument_code,
                                                    rule_variation_name=conditioning_rule_name)

    def mr_forecast(self, instrument_code) -> pd.Series:
        mr_rule_name = self.config.mr['mr_rule']
        return self._get_capped_individual_forecast(instrument_code=instrument_code,
                                                    rule_variation_name=mr_rule_name)

def apply_forecast_filter(forecast, conditioning_forecast):
    conditioning_forecast = conditioning_forecast.reindex(forecast.index, method = "ffill")

    new_values = [forecast_overlay_for_sign(forecast_value, filter_value)
                  for forecast_value, filter_value
                  in zip(forecast.values, conditioning_forecast.values)]
