...
class Test(unittest.TestCase):
    ...
    def test_get_capped_forecast(self):
        self.assertAlmostEqual(self.system.combForecast.get_capped_forecast("EDOLLAR","ewmac8").tail(1).values[0], 0.8712306)
    ...
    def test_get_trading_rule_list(self):
        ...
        system2=System([self.rawdata, self.rules, self.fcs, self.forecast_combine()], self.data, config)
        ...
        system3=System([self.rawdata, self.rules, self.fcs, self.forecast_combine()], self.data, config)
        ...
        system4=System([self.rawdata, self.rules, self.fcs, self.forecast_combine()], self.data, config)
        ...
        system5=System([self.rawdata, self.rules, self.fcs, self.forecast_combine()], self.data, config)
        ans6=system5.combForecast.get_trading_rule_list("EDOLLAR")
        self.assertEqual(ans6, ['ewmac8'])

        #estimated weights - nested dict
        setattr(config, "rule_variations", dict(EDOLLAR = ["ewmac8"], BUND = ["ewmac16"], US10 =["ewmac8", "ewmac16"]))
        system6=System([self.rawdata, self.rules, self.fcs, self.forecast_combine()], self.data, config)
    ...
    def setUpWithEstimatedReturns(self):
        ...
        new_system = System([self.rawdata, self.rules, self.fcs, self.forecast_combine(), Account()], self.data, config)
    ...
    def test_get_returns_for_optimisation(self):
        ...
        print(system.combForecast.get_SR_cost_for_instrument_forecast("EDOLLAR","ewmac8"))
        ...
        system2 = System([self.rawdata, self.rules, self.fcs, self.forecast_combine(), Account()], self.data, config)
