...
class Test(unittest.TestCase):
    ...
    def test_estimated_instrument_weights(self):
        ...
        system2 = System([self.rawdata, self.rules, self.possizing, self.forecast_combine, self.fcs,
                          Account(), self.portfolios()], self.data, config)
    ...
    def test_get_fixed_instrument_diversification_multiplier(self):
        self.assertEqual(self.system.portfolio.get_fixed_instrument_diversification_multiplier().values[-1], 1.2)

    def test_get_notional_position(self):
        self.assertAlmostEqual(self.system.portfolio.get_notional_position("EDOLLAR").values[-1], 1.2231, places=3)

    def test_get_position_method_buffer(self):
        self.assertAlmostEqual(self.system.portfolio.get_position_method_buffer("EDOLLAR").values[-1], 0.12231, places=3)

    def test_get_forecast_method_buffer(self):
        self.assertAlmostEqual(self.system.portfolio.get_forecast_method_buffer("EDOLLAR").values[-1], 0.496673, places=3)
