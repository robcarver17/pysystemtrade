...
class testStage1(SystemStage):
    ...
    @diagnostic()
    def single_instrument_with_keywords_and_flags(self, instrument_code, keyword,  x_flag=3):
        return (7, x_flag)
...
class TestCache(unittest.TestCase):
    def setUp(self):
        ...
        system=System([testStage1(), testStage2()], Data(), Config(dict(instruments=["code", "another_code"])))
        self.system=system
    ...
    def test_single_instrument_no_keywords(self):
        ...
        self.assertEqual(len(self.system.cache._get_pickable_items()),2) # includes cache of instrument list
        ...
        self.assertEqual(len(self.system.cache.cache_ref_list_with_protected_removed(cache_ref_list)),2)
    ...
    def test_single_instrument_with_keywords_and_flags(self):
        self.system.test_stage1.single_instrument_with_keywords_and_flags("code", "keyname", x_flag = True)
        cache_ref = self.system.cache.get_cacherefs_for_stage("test_stage1")[0]
        self.assertEqual(cache_ref.instrument_code, "code")
        self.assertEqual(cache_ref.keyname, "keyname")
    ...
    def test_across_markets_no_keywords(self):
        ...
        self.assertEqual(len(ans),2) ## also includes base_system.get_instrument_list()

        # test deletion across ...
        self.system.test_stage1.single_instrument_no_keywords("code") ## this shouldn't be deleted ...
    ...
    def test_protection_and_deletion_across(self):
        ...
        cache_refs = self.system.cache.get_items_with_data()
        self.assertEqual(len(cache_refs),5) # includes base system get_instruments

        self.system.cache.delete_items_across_system()
        cache_refs = self.system.cache.get_items_with_data()
        self.assertEqual(len(cache_refs),3) # unprotected in stage, and across base_system, have gone

        cache_refs = self.system.cache.get_cache_refs_across_system()
        self.assertEqual(len(cache_refs),1) # protected across system
        cache_ref = cache_refs[0]
        self.assertEqual(cache_ref.itemname, "across_markets_protected") # make sure right one
    ...
    def test_protection_and_deletion_for_code(self):
        ...
        cache_refs = self.system.cache.get_cacherefs_for_stage("test_stage1")
        self.assertEqual(len(cache_refs), 3)  # unprotected 'code' in stage is gone

        self.system.cache.delete_items_for_stage("test_stage1")
        cache_refs = self.system.cache.get_cacherefs_for_stage("test_stage1")
        self.assertEqual(len(cache_refs), 2)  # unprotected 'another_code' in stage is gone

        self.system.cache.delete_items_for_instrument("code", delete_protected=True)
        cache_refs = self.system.cache.get_cacherefs_for_stage("test_stage1")
        self.assertEqual(len(cache_refs), 1)  # just one left 'another code' protected
        cache_ref = cache_refs[0]
        self.assertEqual(cache_ref.itemname, "single_instrument_protected")  # make sure right one
        self.assertEqual(cache_ref.instrument_code, "another_code")  # make sure right one
    ...
    def test_across_stages(self):
        ...
        self.system.test_stage1.single_instrument_with_keywords("another_code", "a_rule")
        self.system.test_stage2.single2_instrument_no_keywords("code")
        self.system.test_stage2.single_instrument_with_keywords("another_code", "a_rule")
        ...
        itemnames.sort()
        self.assertEqual(itemnames, ['single2_instrument_no_keywords', 'single_instrument_with_keywords'])
    ...
    def test_filtering(self):
        ...
        self.assertEqual(len(cache_refs.filter_by_itemname("single_instrument_with_keywords")),2)
        self.assertEqual(len(cache_refs.filter_by_stage_name("test_stage1")),3)
        ...
        items.sort()
        self.assertEqual(items, ["get_instrument_list","single2_instrument_no_keywords", "single_instrument_no_keywords","single_instrument_with_keywords"])

        stage_names = cache_refs.unique_list_of_stage_names()
        stage_names.sort()
        self.assertEqual(stage_names, ["base_system", "test_stage1", "test_stage2"])
