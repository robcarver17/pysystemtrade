...
class Test(unittest.TestCase):
    ...
    def testRules(self):
        ...
        self.assertAlmostEqual(ans.iloc[-1][0], 0.954941033, 5)
    ...
    def testinitTradingRules(self):
        ...
        assert (rule.function, rule.data, rule.other_args)==(rule2.function, rule2.data, rule2.other_args)
        
        rule3=TradingRule(ewmac_forecast_with_defaults, ["data.get_instrument_price"], dict(Lfast=50, Lslow=200))
        
        assert rule3.data==["data.get_instrument_price"]
        
        rule4=TradingRule(ewmac_forecast_with_defaults, "data.get_instrument_price", dict(Lfast=50, Lslow=200))

        assert rule4.data==["data.get_instrument_price"]
        
        try:
            rule4=TradingRule(ewmac_forecast_with_defaults, "data.get_instrument_price")
        ...
        rule7=TradingRule("systems.provided.example.rules.ewmac_forecast_with_defaults", [], dict(Lfast=50, Lslow=200))
        assert rule7.function==rule.function
        
        try:
            rule8=TradingRule("not.a.real.rule.just.a.string.of.arbitrary.text", [], dict(Lfast=50, Lslow=200))
        ...
        rule13=TradingRule("systems.provided.example.rules.ewmac_forecast_with_defaults", data="data.get_pricedata")
    ...
    def testCallingTradingRule(self):
        ...
        ans=rule.call(system, "EDOLLAR")
        self.assertAlmostEqual(ans.iloc[-1][0], 5.603808, 5)
