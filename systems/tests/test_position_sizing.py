...
class Test(unittest.TestCase):
    ...
    def test_get_combined_forecast(self):
        self.assertAlmostEqual(self.system.positionSize.get_combined_forecast("EDOLLAR").values[-1], 2.462610227)

    def test_get_price_volatility(self):
        self.assertAlmostEqual(self.system.positionSize.get_price_volatility("EDOLLAR").values[-1], 0.059789159138)

        ## now without rawdata, should default to calculate on adj price
        system2 = System([self.rules, self.fcs, self.forecast_combine, self.position_sizing()], self.data, self.config)
        self.assertAlmostEqual(system2.positionSize.get_price_volatility("EDOLLAR").values[-1], 0.059723565)
    ...
    def test_get_daily_cash_vol_target(self):
        ...
        system2=System([self.rawdata, self.rules, self.fcs, self.forecast_combine, self.position_sizing()], self.data)
    ...
    def test_get_fx_rate(self):
        self.assertEqual(self.system.positionSize.get_fx_rate("EDOLLAR").values[-1],0.6607594769427981)
        self.assertAlmostEqual(self.system.positionSize.get_fx_rate("BUND").values[-1],0.72446329811485333)

    def test_get_block_value(self):
        self.assertAlmostEqual(self.system.positionSize.get_block_value("EDOLLAR").values[-1], 2449.6875)

    def test_get_instrument_currency_vol(self):
        self.assertAlmostEqual(self.system.positionSize.get_instrument_currency_vol("EDOLLAR").values[-1], 146.46475577626)

    def test_get_instrument_value_vol(self):
        self.assertAlmostEqual(self.system.positionSize.get_instrument_value_vol("EDOLLAR").ffill().values[-1], 96.777975417280)

    def test_get_get_volatility_scalar(self):
        self.assertAlmostEqual(self.system.positionSize.get_volatility_scalar("EDOLLAR").ffill().values[-1], 10.33292952955)

    def test_get_subsystem_position(self):
        self.assertAlmostEqual(self.system.positionSize.get_subsystem_position("EDOLLAR").values[-1], 2.5445977941854627)
