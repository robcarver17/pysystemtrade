...
class cacheRef(object):
    ...
    """
    def __init__(self, stage_name, itemname, instrument_code=ALL_KEYNAME, flags="",
                 keyname=""):
    ...
    def __repr__(self):
        ...
        return "%s in %s for instrument %s %s%s" % (self.itemname, self.stage_name, self.instrument_code, keystring, self.flags)
...
class listOfCacheRefs(list):
    ...
    def filter_by_stage_name(self, stage_name):
        new_list = [cache_ref for cache_ref in self if cache_ref.stage_name == stage_name ]
    ...
    def filter_by_instrument_code(self, instrument_code):
        new_list = [cache_ref for cache_ref in self if cache_ref.instrument_code == instrument_code]
...
class systemCache(dict):
    ...
    def __repr__(self):
        if self.are_we_caching():
            list_of_elements = ", ".join([cache_ref.__repr__() for cache_ref in self.get_items_with_data()])
    ...
    def _get_protected_items(self):
        ...
        protected_cache_refs = [cache_ref for cache_ref in self.get_items_with_data() if self[cache_ref].protected()]
    ...
    def _get_pickable_items(self):
        ...
        pickable_cache_refs = [cache_ref for cache_ref in self.get_items_with_data() if self[cache_ref].can_be_pickled()]
    ...
    def cache_ref_list_with_protected_removed(self, cache_ref_list):
        ...
        cache_ref_list = [cache_ref for cache_ref in cache_ref_list if not self[cache_ref].protected()]
    ...
    def delete_items_for_stage(self, stage_name, delete_protected=False):
        ...
        cache_ref_list = self.get_cacherefs_for_stage(stage_name)
        self.delete_elements_in_cache_ref_list( cache_ref_list, delete_protected = delete_protected)
    ...
    def _delete_elements_in_cache_ref_list_dangerous(self, cache_ref_list):
        ...
        ans = [self._delete_single_element_from_cache_dangerous(cache_ref) for cache_ref in cache_ref_list]
    ...
    def _set_item_in_cache(self, value, cache_ref, protected = False, not_pickable = False):
        """
        ...
        self[cache_ref] = cacheElement(value, protected = protected, not_pickable=not_pickable)



    def _get_item_from_cache(
            self, cache_ref):
        """
    ...
    def calc_or_cache(self, func, this_stage, *args, protected=False,
                                 not_pickable=False, instrument_classify=True, **kwargs):
        """
        ...
        if instrument_classify:
            list_of_codes = self.get_instrument_list() # needed to identify instrument_code amongst args
        else:
            ## if we're calling from the base system we don't want infinite recursion
            list_of_codes = []

        (instrument_code, keyname) = resolve_args_to_code_and_key(args,
                                                                      list_of_codes)  # instrument involved, and/or other keys eg rule name
        flags = resolve_kwargs_to_str(kwargs) # used mostly in accounts, eg to identify delayed returns

        cache_ref = cacheRef(stage_name, itemname, instrument_code, flags=flags,
                             keyname=keyname)
        ...
        if value is MISSING_FROM_CACHE:
            ...
            value = func(this_stage,  *args, **kwargs)
            self._set_item_in_cache(value, cache_ref, protected=protected, not_pickable=not_pickable)
...
def resolve_kwargs_to_str(kwargs):
    ...
    long_flag_string = [resolve_individual_flag(single_flag, kwargs) for single_flag in kwargs.keys()]
...
def stage_access_cache_decorator(protected=False, not_pickable=False):
    ...
    def decorate(func):
        ...
        def wrapper(self,  *args, **kwargs):
            ...
            ans = system.cache.calc_or_cache(func,
                                             this_stage, *args, protected=protected,
                                             not_pickable=not_pickable, instrument_classify=True, **kwargs)
...
def base_system_cache(protected=False, not_pickable=False):
    ...
    def decorate(func):
        ...
        def wrapper(self,  *args, **kwargs):
            ...
            ans = system.cache.calc_or_cache(func,
                                             system, *args, protected=protected,
                                             not_pickable=not_pickable, instrument_classify=False, **kwargs)
