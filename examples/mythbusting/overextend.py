...
def bin_fit(x, y, buckets=3):
    ...
    for binidx in range(len(binlimits))[1:]:
        ...
        y_in_bin = [y[idx] for idx in range(len(y)) if x[idx] >= lower_bin_x and x[
            idx] < upper_bin_x]
...
def get_scatter_data_for_code_forecast(
        system, instrument_code, rule_name, startdate=None, enddate=None, return_period=5):
    ...
    (forecast, norm_data) = align_to_joint(forecast[
        startdate:enddate], norm_data[startdate:enddate], ffill=(True, False))
...
def do_a_little_plot(system):
    ...
    for instrument_code in instrument_list:
        ...
        all_scatter['returns'] = all_scatter[
            'returns'] + this_instrument_data[0]
        all_scatter['forecast'] = all_scatter[
            'forecast'] + this_instrument_data[1]

    (returns, forecast) = clean_data(
        all_scatter['returns'], all_scatter['forecast'])
...
def fit_a_filter_datewise(system, rule_name, instrument_code=None,
                          return_period=5, date_method="expanding", rollyears=999, buckets=3):
    """
    ...
    fit_dates = generate_fitting_dates([system.rules.get_raw_forecast(
        instrument_code, rule_name) for instrument_code in instrument_list], date_method, rollyears)
...
def fit_a_filter(system, rule_name, instrument_list,
                 start_date=None, end_date=None, return_period=5, buckets=3):

    all_scatter = dict(returns=[], forecast=[])
    for instrument_code in instrument_list:
        this_instrument_data = get_scatter_data_for_code_forecast(
            system, instrument_code, rule_name, start_date, end_date, return_period)
        all_scatter['returns'] = all_scatter[
            'returns'] + this_instrument_data[0]
        all_scatter['forecast'] = all_scatter[
            'forecast'] + this_instrument_data[1]
...
def filtering_function(raw_forecast, x_bins=None,
                       fit_y=None, startdate=None, enddate=None):
    """
