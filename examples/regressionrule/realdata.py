...
ans=system.combForecast.get_forecast_correlation_matrices(instrument_code).corr_list
print(ans[-1])
...
for instrument_code in system.get_instrument_list():
    for timewindow in WINDOWS_TO_USE:
        ...
        scaling = system.forecastScaleCap.get_forecast_scalar(instrument_code, rule_name).values[-1]
        turnover = system.accounts.forecast_turnover(instrument_code, rule_name)
        cost_sr = system.accounts.get_SR_cost_for_instrument_forecast(instrument_code, rule_name)

        print("*** %s Window length %d scalar %.3f turnover %.3f costs %.6f" % (instrument_code, timewindow, scaling, turnover, cost_sr))
