...
def create_data_for_random_system():
    ...
    for Volscale in VOLS_TO_USE:
        for Tlength in LENGTHS_TO_USE:
            ...
            data.generate_random_data(instrument_code, Nlength, Tlength, Xamplitude, Volscale)
...
for Volscale in VOLS_TO_USE:
    for Tlength in LENGTHS_TO_USE:
        ...
        for timewindow in WINDOWS_TO_USE:
            ...
            sr=system.accounts.pandl_for_instrument_forecast(instrument_code, rule_name).sharpe()
            turnover=system.accounts.forecast_turnover(instrument_code, rule_name)

            print("*** Vol scale %.2f Trend length %d Window length %d SR %.3f turnover %.3f" % (Volscale, Tlength, timewindow, sr, turnover))
