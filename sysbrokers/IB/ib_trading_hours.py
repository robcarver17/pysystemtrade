...
def parse_trading_hours_string(trading_hours_string, adjustment_hours =0, one_off_adjustment=[0,0]):
    day_by_day = trading_hours_string.split(";")
    list_of_open_times = [parse_trading_for_day(string_for_day,
                                                adjustment_hours =adjustment_hours,
                                                one_off_adjustment= one_off_adjustment)
                          for string_for_day in day_by_day]

    list_of_open_times = [open_time for open_time in list_of_open_times if open_time is not None]

    return list_of_open_times

def parse_trading_for_day(string_for_day, adjustment_hours =0, one_off_adjustment = [0,0]):
    ...
    start_dt = parse_phrase(start_phrase, adjustment_hours =adjustment_hours, additional_adjust =adjust_start)
    end_dt = parse_phrase(end_phrase, adjustment_hours = adjustment_hours, additional_adjust = adjust_end)
