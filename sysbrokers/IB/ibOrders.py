...
from sysbrokers.IB.ib_translate_broker_order_objects import create_broker_order_from_control_object
...
class ibOrderWithControls(orderWithControls):
    def __init__(self,  control_object, ibconnection, broker_order = None, instrument_code=None,
                 ticker_object=None, reference_of_controlling_algo=None
                 ):
        if broker_order is None:
            ## This might happen if for example we are getting the orders from IB
            self._order = create_broker_order_from_control_object(control_object, instrument_code)
        else:
    ...
    def write_placed_broker_trade_info_in_broker_order(self):
        ib_broker_order = create_broker_order_from_control_object(self.control_object, self.order.instrument_code)
        updated_broker_order = write_placed_broker_trade_info_in_broker_order(self.order, ib_broker_order)
    ...
    def update_order(self):
        ...
        ## Basically copies across the details from the control object that are likely to be updated
        self._ibconnection.refresh()
        ib_broker_order = create_broker_order_from_control_object(self.control_object, self.order.instrument_code)
        updated_broker_order = add_trade_info_to_broker_order(self.order, ib_broker_order)
...
class ibOrdersData(brokerOrderStackData):
    ...
    def get_list_of_broker_orders(self, account_id=arg_not_supplied):
        ...
        """
        list_of_control_objects = self.get_list_of_broker_control_orders(account_id = account_id)
        order_list = [order_with_control.order for order_with_control in list_of_control_objects]

        return order_list

    def get_dict_of_broker_control_orders(self, account_id=arg_not_supplied):
        control_order_list = self.get_list_of_broker_control_orders(account_id=account_id)
        dict_of_control_orders = dict([(control_order.order.broker_tempid, control_order)
    ...
    def get_list_of_broker_control_orders(self, account_id=arg_not_supplied):
        ...
        list_of_raw_orders_as_trade_objects = self.ibconnection.broker_get_orders(account_id=account_id)
        control_order_list = [self.create_broker_control_order_object(broker_trade_object_results)
                              for broker_trade_object_results
                      in list_of_raw_orders_as_trade_objects]
    ...
    def create_broker_control_order_object(self, broker_trade_object_results):
        ...
        instrument_code=self.futures_contract_data.\
            get_instrument_code_from_broker_code(broker_trade_object_results.ib_instrument_code)
        broker_order_with_controls = ibOrderWithControls(broker_trade_object_results,
                                                         self.ibconnection, instrument_code = instrument_code)
    ...
    def get_dict_of_orders_from_storage(self):
        ...
        order_dict = dict([(key, order_with_control.order)
    ...
    def get_dict_of_control_orders_from_storage(self):
        ...
        __ = [order_with_control.update_order() for order_with_control in dict_of_orders_with_control.values()]
    ...
    def match_db_broker_order_to_order_from_brokers(self, broker_order_to_match):
        matched_control_order = self.match_db_broker_order_to_control_order_from_brokers(broker_order_to_match)
    ...
    def match_db_broker_order_to_control_order_from_brokers(self, broker_order_to_match):
        """
        ...
        matched_control_order = match_control_order_from_dict(dict_of_stored_control_orders, broker_order_to_match)
        ...
        dict_of_broker_control_orders = self.get_dict_of_broker_control_orders(account_id=account_id)
        matched_control_order = match_control_order_from_dict(dict_of_broker_control_orders,  broker_order_to_match)
        if matched_control_order is not missing_order:
            return matched_control_order

        ## Match on permid
        matched_control_order = match_control_order_on_permid(dict_of_broker_control_orders, broker_order_to_match)
    ...
    def cancel_order_on_stack(self, broker_order):
        ...
        matched_control_order = self.match_db_broker_order_to_control_order_from_brokers(broker_order)
        if matched_control_order is missing_order:
            return failure
    ...
    def check_order_is_cancelled(self, broker_order):
        ...
        if matched_control_order is missing_order:
            return failure
        cancellation_status = self.check_order_is_cancelled_given_control_object(matched_control_order)
...
def write_placed_broker_trade_info_in_broker_order(broker_order, broker_order_from_trade_object):
    new_broker_order = copy(broker_order)
    keys_to_replace = ['broker_permid', 'broker_account', 'broker_clientid',
                       'commission', 'broker_permid', 'broker_tempid']
...
def add_trade_info_to_broker_order(broker_order, broker_order_from_trade_object):
    ...
    if broker_order_is_filled:
        new_broker_order.fill_order(broker_order_from_trade_object.fill, broker_order_from_trade_object.filled_price,
...
def match_control_order_on_permid(dict_of_broker_control_orders, broker_order_to_match):
    list_of_broker_control_orders = dict_of_broker_control_orders.values()
    list_of_broker_orders = [control_order.order for control_order in list_of_broker_control_orders]
...
def match_control_order_from_dict( dict_of_broker_control_orders,  broker_order_to_match):

    matched_control_order_from_dict = dict_of_broker_control_orders.get(broker_order_to_match.broker_tempid, missing_order)
