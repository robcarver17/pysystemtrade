...
from sysquant.optimisation.weights import estimatesWithPortfolioWeights, portfolioWeights
...
def optimiser_for_method(method: str,
                         estimates: Estimates,
                         **weighting_args) -> estimatesWithPortfolioWeights:
    ...
    weights_with_estimates_for_valid_data = call_optimiser(method,
    ...
    weights = portfolioWeights.from_list_of_subportfolios([weights_for_valid_data,
                                                           weights_for_missing_data])

    weights_with_estimates_for_valid_data.weights = weights

    return weights_with_estimates_for_valid_data

def weights_and_estimates_with_no_valid_data(estimates: Estimates) -> estimatesWithPortfolioWeights:
    weights_with_no_valid_data = portfolioWeights.allnan([])
    weights_with_estimates_for_valid_data = estimatesWithPortfolioWeights(weights=weights_with_no_valid_data,
                                                                          estimates=estimates)

    return  weights_with_estimates_for_valid_data

def call_optimiser(method: str,
                         estimates_with_only_valid_data: Estimates,
                         **weighting_args) -> estimatesWithPortfolioWeights:
    ...
    weights_with_estimates_for_valid_data = optimisation_function(estimates_with_only_valid_data,
                          **weighting_args)
