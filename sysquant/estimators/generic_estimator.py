...
class exponentialEstimator(object):
    def __init__(self, data,
        ...
        calculations = self.perform_calculations(data,
...
class genericEstimator(object):
    ...
    def calculate_estimate_for_period(self, fit_period: fitDates):
        ...
        if self.using_exponent:
            estimate = self.calculate_estimate_using_exponential_data_for_period(fit_period)
        else:
    ...
    def calculate_estimate_using_exponential_data_for_period(self, fit_period: fitDates):
    ...
    def get_exponential_estimator_for_entire_dataset(self) -> exponentialEstimator:
        ...
        if exponential_estimator is None:
            exponential_estimator = self.calculate_exponential_estimator_for_entire_dataset()
            self._stored_exponential_estimator = exponential_estimator

        return exponential_estimator

    def calculate_exponential_estimator_for_entire_dataset(self) -> exponentialEstimator:
        kwargs_for_estimator = self.kwargs_for_estimator
        exponential_estimator = \
            exponentialEstimator(self.data, **kwargs_for_estimator)
