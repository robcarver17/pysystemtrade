...
class spreadsForInstrumentData(baseData):
    ...
    def delete_spreads(self, instrument_code: str, are_you_sure: bool=False):
        if are_you_sure:
            if self.is_code_in_data(instrument_code):
                self._delete_spreads_without_any_warning_be_careful(
                    instrument_code
                )
                self.log.terse(
                    "Deleted spread data for %s" %
                    instrument_code, instrument_code = instrument_code)

            else:
                # doesn't exist anyway
                self.log.warn(
                    "Tried to delete non existent spreads for %s"
                    % instrument_code, instrument_code = instrument_code
                )
        else:
            self.log.error(
    ...
    def add_spreads(
        ...
        self._add_spreads_without_checking_for_existing_entry(
            instrument_code, spreads
        )
    ...
    def _delete_spreads_without_any_warning_be_careful(self,
            instrument_code: str):
        raise NotImplementedError(USE_CHILD_CLASS_ERROR)
