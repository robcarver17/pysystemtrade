...
def from_contract_numbers_to_contract_string(new_year_number, new_month_number, new_day_number=NO_DAY_PASSED):
...
class contractDate(object):
    ...
    def __init__(self, contract_date, expiry_date=NO_EXPIRY_DATE_PASSED, approx_expiry_offset=0):
        """
        ...
        ## The approx expiry offset must be set first before we try and define the expiry
    ...
    def expiry_date(self, expiry_date):
        ...
        if expiry_date is NO_EXPIRY_DATE_PASSED:
            ...
            new_expiry_date = approx_expiry_date + datetime.timedelta(days = offset_in_days)
            expiry_date = (new_expiry_date.year, new_expiry_date.month, new_expiry_date.day)
    ...
    def as_dict(self):
        ## we do this so that we can init the object again from this with the correct length of contract_date
        if self._only_has_month:
            contract_date = self.contract_date[YYYYMM_SLICE] # remove trailing zeros
        else:
            contract_date = self.contract_date

        expiry_date = (self.expiry_date.as_tuple())

        return dict(expiry_date=expiry_date, contract_date=contract_date, approx_expiry_offset = self._approx_expiry_offset)
    ...
    def create_from_dict(contractDate, results_dict):
        ...
        return contractDate(results_dict['contract_date'], expiry_date=expiry_date)
    ...
    @classmethod
    def contract_date_from_numbers(contractDate, new_year_number, new_month_number, new_day_number=NO_DAY_PASSED,
                                   expiry_date = NO_EXPIRY_DATE_PASSED, approx_expiry_offset = 0):
        """
        ...
        new_contract_date_as_string = from_contract_numbers_to_contract_string(new_year_number, new_month_number, new_day_number)

        return contractDate(new_contract_date_as_string, expiry_date=expiry_date, approx_expiry_offset=approx_expiry_offset)
