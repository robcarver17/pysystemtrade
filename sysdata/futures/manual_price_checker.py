...
def manual_price_checker(old_data_passed, new_data_passed,
    ...
    while data_iterating:
        ...
        spike_present, first_spike = spike_check_merged_data(merge_status, first_date_in_new_data, merged_data,
                                                column_to_check=column_to_check)
        ...
        previous_value = merged_data[column_to_check][position_of_spike_in_mergedata-1]


        ## Get input
        _show_last_bit_of_data(merged_data, position_of_spike_in_mergedata)
        value_to_use, adjust_old_and_new_data = _get_new_value(original_value, previous_value,
        ...
        if delta_columns is not arg_not_supplied:
            ...
            new_data = _adjust_delta_columns(new_data, value_to_use, original_value, first_spike, delta_columns)

        if adjust_old_and_new_data:
            ## Change old_data so it now includes the spike checked bit
            old_data, new_data = _adjust_old_and_new_data(old_data, new_data, position_of_spike_in_newdata)
            if len(new_data)==0:
                data_iterating = False
                break
    ...
    old_data = merged_data[:original_last_date_of_old_data] ## Not used, but for tidiness
...
def _get_new_value(original_value, previous_value, col_date_str='', total_msg_str=''):
    ...
    while waiting_for_valid_input:
        print("\n")
        print("Value %f of %s is a big change from previous value of %f" %
              (original_value, col_date_str, previous_value))
        print("<return> to accept, <space><return> for previous value,  <float><return> for a new value")
        print(total_msg_str)
        ...
        if result == '':
            ...
            ## User input
            ## NOTE: user input will get checked by spike checker, so if we screw up that's fine
            try:
...
def _adjust_old_and_new_data(old_data, new_data, position_of_spike_in_newdata):
    ...
    _, _, merge_of_old_and_checked_new_data = merge_newer_data_no_checks(old_data, checked_new_data)
...
def _adjust_delta_columns(new_data, value_to_use, original_value, first_spike, delta_columns):
...
def _resolve_and_check_columns(old_data, column_to_check=arg_not_supplied, delta_columns=arg_not_supplied):
    ...
    if delta_columns is not arg_not_supplied:
        delta_col_str = "%s will be adjusted in line with change to %s" % (str(delta_columns), column_to_check)
