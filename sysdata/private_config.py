...
from systems.defaults import get_default_config_key_value, get_system_defaults, DEFAULT_FILENAME
...
def get_private_config_key_value(key_name, private_config_dict = arg_not_supplied, raise_error=False):
...
def get_private_then_default_key_value(key_name,
    ...
    key_value = get_private_config_key_value(key_name, private_config_dict=private_config_dict)
...
def get_list_of_private_then_default_key_values(list_of_key_names, fail_if_any_missing = True):
    result_dict = {}
    system_defaults_dict = get_system_defaults()
    private_config_dict = get_private_config()

    for key_name in list_of_key_names:
        key_value = get_private_then_default_key_value(key_name,
...
def get_list_of_private_config_values(list_of_key_names, fail_if_any_missing=True):
    result_dict = {}
    private_config_dict = get_private_config()
