...
from syscore.objects import arg_not_supplied, missing_data, success, failure, resolve_function
...
class timedEntry(object):
    ...
    def _resolve_args_normally(self, args, date):
        ...
        try:
            ...
            raise Exception("Expecting to be passed arguments of length %d to match %s, instead got %d arguments"
    ...
    def check_match(self, another_entry):
        ...
        try:
            ...
            raise Exception("Parameters for %s (%s) don't match with %s" % self._name_(), my_args, another_args)
...
class listOfEntries(list):
    ...
    def from_list_of_dict(cls, list_of_dict):
        ...
        list_of_class_entries = [entry_class.from_dict(entry_as_dict)
    ...
    def as_pd_df(self):
        ...
        if len(self)==0:
            return missing_data
        list_of_dates, dict_of_lists, = self._as_list_of_lists()
...
class listOfEntriesData(baseData):
    ...
    def _update_entry_for_args_dict(self, new_entry, args_dict):
        ...
        try:
            ...
            self.log.warn("Error %s when updating for %s with %s" % (str(e), str(args_dict), str(new_entry)))
            return failure

        self._write_series_for_args_dict(args_dict, entry_series, data_class_name=data_class_new_entry)
    ...
    def _delete_last_entry_for_args_dict(self, args_dict, are_you_sure=False):
        ...
        try:
            ...
            self.log.warn("Can't delete last entry for %s, as none present" % str(args_dict))
            return failure
    ...
    def _get_series_for_args_dict(self, args_dict):
        data_class, series_as_dict = self._get_series_dict_and_class_for_args_dict(args_dict)
    ...
    def _get_series_dict_and_class_for_args_dict(self, args_dict):
        ...
        series_as_dict_with_data_class = self._get_series_dict_with_data_class_for_args_dict(args_dict)
        data_class_name = self._get_data_class_name_for_args_dict(args_dict,
                                                                 series_as_dict_with_data_class = series_as_dict_with_data_class)
        __, series_as_dict = series_as_dict_with_data_class
        data_class = resolve_function(data_class_name)

        return data_class, series_as_dict

    def _get_data_class_name_for_args_dict(self, args_dict, series_as_dict_with_data_class = arg_not_supplied):
    ...
    def _write_series_for_args_dict(self, args_dict, entry_series, data_class_name = arg_not_supplied):
        entry_series_as_list_of_dicts = entry_series.as_list_of_dict()
        if data_class_name is arg_not_supplied:
            data_class_name = self._get_data_class_name_for_args_dict(args_dict)

        self._write_series_dict_for_args_dict(args_dict, entry_series_as_list_of_dicts, data_class_name)
