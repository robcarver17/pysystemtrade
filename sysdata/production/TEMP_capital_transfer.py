...
from sysdata.production.TEMP_old_capital_objects import get_old_capital, delete_old_total_capital, get_dict_of_capital_by_strategy, delete_old_capital_for_strategy
...
def transfer_global_capital():
    ...
    sure = true_if_answer_is_yes("Happy to delete current 'new' capital and replace with transferred 'old'?")
    if sure:
...
def transfer_strategy_capital():
    ...
    for strategy_name, old_capital_for_strategy in original_strategy_capital_as_dict.items():
        try:
            transfer_capital_for_specific_strategy(strategy_name, old_capital_for_strategy)
        except:
...
def transfer_capital_for_specific_strategy(strategy_name, old_capital_for_strategy):
    ...
    sure = true_if_answer_is_yes("Happy to delete current 'new' capital and replace with transferred 'old'?")
    if sure:
        new_capital_data.delete_all_capital_for_strategy(strategy_name,
                                                         are_you_really_sure=True)
        try:
            new_capital_data.update_capital_pd_df_for_strategy(strategy_name,
                                                               old_capital_for_strategy.to_frame())
        except:
            print("Problem with capital adding %s - probably a data artifact can ignore" % str(old_capital_for_strategy))
