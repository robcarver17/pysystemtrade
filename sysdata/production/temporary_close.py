...
class temporaryCloseData(baseData):
    ...
    def add_stored_position_limit(self, position_limit_for_instrument: positionLimitForInstrument):
        if self.does_instrument_have_position_limit_stored(position_limit_for_instrument.key):
            raise Exception("Need to clear_stored_position_limit before adding a new one for %s" % position_limit_for_instrument.key)
