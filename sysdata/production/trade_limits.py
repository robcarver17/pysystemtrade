...
class tradeLimit(object):
    ...
    def __init__(self, trade_limit, strategy_name = "",
                 instrument_code= "",
                 period_days=1, trades_since_last_reset = 0, last_reset_time=arg_not_supplied):
    ...
    def as_dict(self):
        result_dict = dict(trade_limit = self.trade_limit,
    ...
    def remove_trade(self, trade_to_remove):
        ...
        self._trades_since_last_reset = max(self._trades_since_last_reset - abs_trade_to_remove, 0)
...
class listOfTradeLimits(list):
    ...
    def remove_trade(self, trade_to_remove):
        result = [trade_limit.remove_trade(trade_to_remove) for trade_limit in self]
...
class tradeLimitData(object):
    ...
    def default_object(self, strategy_name, instrument_code, period_days):
        return tradeLimit(999999, strategy_name=strategy_name, instrument_code=instrument_code,
    ...
    def _get_list_of_all_relevant_trade_limits(self, strategy_name, instrument_code):
        list_of_instrument_trade_limits = \
            self.get_list_of_trade_limits_for_instrument(instrument_code)
        list_of_strategy_instrument_trade_limits = \
            self.get_list_of_trade_limits_for_strategy_instrument(strategy_name, instrument_code)

        combined_list = listOfTradeLimits(list_of_instrument_trade_limits + list_of_strategy_instrument_trade_limits)

        return combined_list

    def get_list_of_trade_limits_for_instrument(self, instrument_code):
        return self.get_list_of_trade_limits_for_strategy_instrument("", instrument_code)

    def get_list_of_trade_limits_for_strategy_instrument(self, strategy_name, instrument_code):
        all_keys = self._get_all_limit_keys()
        trade_limits = [self._get_trade_limit_object(key[0], key[1], key[2]) for key in all_keys
    ...
    def _update_list_of_trade_limits(self, list_of_trade_limits):
        result = [self._update_trade_limit_object(trade_limit_object)
    ...
    def update_instrument_limit_with_new_limit(self, instrument_code, period_days, new_limit):
        self.update_instrument_strategy_limit_with_new_limit("", instrument_code, period_days, new_limit)
    ...
    def _get_trade_limit_object(self, strategy_name, instrument_code, period_days):
        trade_limit_object = self._get_trade_limit_object_or_missing_data(strategy_name, instrument_code, period_days)
        if trade_limit_object is missing_data:
            return self.default_object(strategy_name, instrument_code, period_days)
        else:
            return trade_limit_object

    def _get_trade_limit_object_or_missing_data(self, strategy_name, instrument_code, period_days):
        trade_limit_object = self._limits.get((strategy_name, instrument_code, period_days), missing_data)
        return trade_limit_object
    ...
    def _update_trade_limit_object(self,  trade_limit_object):
        ...
        self._limits[(strategy_name, instrument_code, period_days)] = trade_limit_object
