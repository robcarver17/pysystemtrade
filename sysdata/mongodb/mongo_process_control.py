...
class mongoControlProcessData(controlProcessData):
    ...
    def __init__(self, mongo_db = None, log=logtoscreen("mongoControlProcessData")):

        super().__init__(log=log)

        self._mongo = mongoConnection(PROCESS_CONTROL_COLLECTION, mongo_db=mongo_db)
    ...
    def _get_control_for_process_name_without_default(self, process_name):
        result_dict = self._mongo.collection.find_one(dict(process_name=process_name))
