...
from sysdata.production.trade_limits import tradeLimitData, tradeLimit, listOfTradeLimits
...
class mongoTradeLimitData(tradeLimitData):
    ...
    def __init__(self, mongo_db=None, log=logtoscreen("mongoTradeLimitData")):
        ...
        self._mongo = mongoConnection(LIMIT_STATUS_COLLECTION, mongo_db=mongo_db)
    ...
    def _get_trade_limit_object_or_missing_data(self, strategy_name, instrument_code, period_days):
        result_dict = self._mongo.collection.find_one(dict(strategy_name=strategy_name,
    ...
    def _update_trade_limit_object(self, trade_limit_object):
        ...
        if self._get_trade_limit_object_or_missing_data(strategy_name, instrument_code, period_days) is missing_data:
