...
class mongoTradeLimitData(tradeLimitData):
    ...
    def __init__(self, mongo_db=None, log=logtoscreen("mongoTradeLimitData")):
        ...
        self._mongo_data = mongoDataWithMultipleKeys(LIMIT_STATUS_COLLECTION, mongo_db=mongo_db)
    ...
    def __repr__(self):
        return \
            "Data connection for trade limit data, mongodb %s" % (str(self.mongo_data))


    def _get_trade_limit_as_dict_or_missing_data(
        self, instrument_strategy: instrumentStrategy, period_days: int) -> dict:

        instrument_strategy_key = instrument_strategy.key
        dict_of_keys = {INSTRUMENT_STRATEGY_KEY: instrument_strategy_key,
                        PERIOD_KEY: period_days}

        result_dict = self.mongo_data.get_result_dict_for_dict_keys(dict_of_keys)

        if result_dict is missing_data:
            result_dict = self._get_old_style_trade_limit_as_dict_or_missing_data(instrument_strategy, period_days)
        return result_dict

    def _get_old_style_trade_limit_as_dict_or_missing_data(
        self, instrument_strategy: instrumentStrategy, period_days: int) -> dict:

        dict_of_keys = {LEGACY_INSTRUMENT_KEY: instrument_strategy.instrument_code,
    ...
    def _delete_old_style_data(self, instrument_strategy_key: str, period_days: int):
        ...
        dict_of_keys = {LEGACY_STRATEGY_KEY: instrument_strategy.strategy_name,
    ...
    def _get_all_limit_keys(self) -> listOfInstrumentStrategyKeyAndDays:
        ...
        list_of_results = [_from_result_dict_to_isd(result_dict) for result_dict in list_of_result_dicts]
