...
class mongoEmailControlData(emailControlData):
    def __init__(self, mongo_db = None, log=logtoscreen("mongoEmailControlData")):
        ...
        self._mongo = mongoConnection(EMAIL_CONTROL_COLLECTION, mongo_db=mongo_db)
    ...
    def get_time_last_email_sent_with_this_subject(self, subject):
        result_as_datetime = self._get_time_last_email_of_type_sent_with_this_subject(subject, LAST_EMAIL_SENT)

        return result_as_datetime

    def get_time_last_warning_email_sent_with_this_subject(self, subject):
        result_as_datetime = self._get_time_last_email_of_type_sent_with_this_subject(subject, LAST_WARNING_SENT)

        return result_as_datetime

    def _get_time_last_email_of_type_sent_with_this_subject(self, subject, type):
        result_dict = self._mongo.collection.find_one(dict(type=type, subject=subject))
    ...
    def _record_date_of_email_type_send(self, subject, type):
        ...
        if result_dict is None:
            object_dict = dict(type=type, subject=subject, datetime=datetime_now)
    ...
    def store_message(self, body, subject):
        ...
        object_dict = dict(type = STORED_MSG, subject = subject, datetime = datetime_now, body = body)
        self._mongo.collection.insert_one(object_dict)
    ...
    def get_stored_messages(self, subject):
        ...
        stored_msgs = [(long_to_datetime(dict['datetime']), dict['subject'], dict['body'])
    ...
    def delete_stored_messages(self, subject=None):
        if subject is None:
            ...
            self._mongo.collection.remove(dict(type = STORED_MSG, subject = subject))
