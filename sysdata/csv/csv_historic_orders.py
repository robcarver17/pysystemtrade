...
import pandas as pd
from sysdata.production.historic_orders import strategyHistoricOrdersData, contractHistoricOrdersData
...
class csvStrategyHistoricOrdersData(strategyHistoricOrdersData):
    ...
    def __init__(self, datapath=arg_not_supplied, log=logtoscreen("csvStrategyPositionData")):
    ...
    def write_orders(self, list_of_orders):
        filename = get_filename_for_package(self._datapath, "%s.csv" % ("strategy_orders"))
...
class csvContractHistoricOrdersData(contractHistoricOrdersData):
    def __init__(self, datapath=arg_not_supplied, log=logtoscreen("csvContractPositionData")):
    ...
    def write_orders(self, list_of_orders):
        filename = get_filename_for_package(self._datapath, "%s.csv" % ("contract_orders"))
        df = from_list_of_orders_to_df(list_of_orders)
        df.to_csv(filename, index_label=DATE_INDEX_NAME)


class csvBrokerHistoricOrdersData( contractHistoricOrdersData):
    def __init__(self, datapath=arg_not_supplied, log=logtoscreen("csvBrokerHistoricOrdersData")):
    ...
    def write_orders(self, list_of_orders):
        filename = get_filename_for_package(self._datapath, "%s.csv" % ("broker_orders"))
        df = from_list_of_orders_to_df(list_of_orders)
        df.to_csv(filename, index_label=DATE_INDEX_NAME)
