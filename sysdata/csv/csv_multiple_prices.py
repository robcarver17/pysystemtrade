...
from sysobjects.multiple_prices import futuresMultiplePrices, list_of_contract_column_names
...
class csvFuturesMultiplePricesData(futuresMultiplePricesData):
    ...
    def __init__(self, datapath: str=arg_not_supplied, log=logtoscreen(
    ...
    def _get_multiple_prices_without_checking(self, instrument_code: str) -> futuresMultiplePrices:
        ...
        for contract_col_name in list_of_contract_column_names:
            instr_all_price_data[contract_col_name] = instr_all_price_data[contract_col_name].apply(
            str_of_int)
    ...
    def _add_multiple_prices_without_checking_for_existing_entry(
        ...
        self.log.msg("Written multiple prices for %s to %s" % (instrument_code, filename), instrument_code=instrument_code)
    ...
    def _read_instrument_prices(self, instrument_code: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.log.warning("Can't find multiple price file %s or error reading" % filename,
