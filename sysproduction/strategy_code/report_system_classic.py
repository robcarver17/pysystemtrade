...
def report_system_classic(data, data_backtest):
    ...
    format_output = []
    report_header = header("Strategy report for %s backtest timestamp %s produced at %s" %
                                (strategy_name, data_backtest.timestamp, str(datetime.datetime.now())))
    format_output.append(report_header)

    unweighted_forecasts_df = get_forecast_matrix(data_backtest,
    ...
    unweighted_forecasts_table = table("Unweighted forecasts", unweighted_forecasts_df_rounded)
    format_output.append(unweighted_forecasts_table)

    # Forecast weights
    forecast_weights_df = get_forecast_matrix_over_code(data_backtest,
    ...
    forecast_weights_table = table("Forecast weights", forecast_weights_df_as_perc_rounded)
    ...
    # Vol calc
    vol_calc_df = get_stage_breakdown_over_codes(data_backtest,
                                                 method_list = [
    ...
    subystem_positions_df = get_stage_breakdown_over_codes(data_backtest,
                                                           method_list=[
                                                            get_block_value,
    ...
    subystem_positions_table = table("Subsystem position", subystem_positions_df_rounded)
    format_output.append(subystem_positions_table)

    # Portfolio position table: ss position, instr weight, IDM, position required
    portfolio_positions_df = get_stage_breakdown_over_codes(data_backtest,
                                   method_list = [get_subsystem_position,
                                                  get_instrument_weights,
                                                  get_idm,
                                                  get_required_portfolio_position])
    portfolio_positions_df_rounded = portfolio_positions_df.round(3)
    portfolio_positions_table = table("Portfolio positions", portfolio_positions_df_rounded)

    format_output.append(portfolio_positions_table)

    # Position vs buffer table: position required, buffers, actual position
    versus_buffers_df = get_stage_breakdown_over_codes(data_backtest,
                                                       method_list=[get_required_portfolio_position,
                                                                    get_lower_buffer,
                                                                    get_upper_buffer])

    instrument_code_list = versus_buffers_df.index
    timestamp_positions = get_position_at_timestamp_df_for_instrument_code_list(data_backtest, data, instrument_code_list)
    current_positions = get_current_position_df_for_instrument_code_list(data_backtest, data, instrument_code_list)
    versus_buffers_and_positions_df = pd.concat([versus_buffers_df, timestamp_positions, current_positions], axis=1)
    versus_buffers_and_positions_df_rounded = versus_buffers_and_positions_df.round(1)
    versus_buffers_and_positions_table = table("Positions vs buffers", versus_buffers_and_positions_df_rounded)
...
def get_forecast_matrix(data_backtest, stage_name = "combForecast", method_name = "get_capped_forecast"):
...
def get_forecast_matrix_over_code(data_backtest, stage_name = "combForecast", method_name = "get_forecast_weights"):
    instrument_codes = data_backtest.system.get_instrument_list()
    trading_rules = data_backtest.system.rules.trading_rules()
    trading_rule_names = list(trading_rules.keys())
    ...
    for instrument_code in instrument_codes:
        ...
        values_by_rule = [value_row.get(rule_name, np.nan) for rule_name in trading_rule_names]
...
configForMethod = namedtuple("ConfigForMethod", ['stage_name',
                             'method_name',
                            'name',
                             'global_bool',
                             'requires_code_bool',
...
daily_returns_vol = configForMethod("rawdata",
...
daily_denom_price = configForMethod("rawdata",
...
rawdata_daily_perc_vol = configForMethod("rawdata",
...
get_combined_forecast = configForMethod("positionSize",
                                        "get_combined_forecast",
                                        "Combined forecast",
                                        False, True, None,
                                        False, None)
...
get_block_value = configForMethod("positionSize",
                                           "get_block_value",
                                           "Block_Value",
                                           False, True, None,
                                  False, None)

get_price_volatility = configForMethod("positionSize",
                                           "get_price_volatility",
                                           "Daily price % vol",
                                           False, True, None,
                                       False, None)
...
get_fx_rate = configForMethod("positionSize",
                                           "get_fx_rate",
                                           "FX",
                                           False, True, None, False, None)


get_instrument_ccy_vol = configForMethod("positionSize",
                                           "get_instrument_currency_vol",
                                           "ICV",
                                           False, True, None, False, None)


get_instrument_value_vol = configForMethod("positionSize",
                                           "get_instrument_value_vol",
                                           "IVV",
                                           False, True, None, False, None)


get_daily_cash_vol_target = configForMethod("positionSize",
                                           "get_daily_cash_vol_target",
                                           "Daily Cash Vol Tgt",
                                           False, False, None, True, 'daily_cash_vol_target')


get_vol_scalar = configForMethod("positionSize",
                                        "get_volatility_scalar",
                                        "Vol Scalar",
                                        False, True, None, False, None)

get_subsystem_position = configForMethod("positionSize",
                                         "get_subsystem_position",
                                         "subsystem_position",
                                         False, True, None, False, None)

get_instrument_weights = configForMethod("portfolio",
                                         "get_instrument_weights",
                                         "instr weight",
                                         False, False, None, False, None)

get_idm = configForMethod("portfolio",
                          "get_instrument_diversification_multiplier",
                          "IDM",
                          True, False, None, False, None)

get_required_portfolio_position = configForMethod("portfolio",
                                                  "get_notional_position",
                                                  "Notional position",
                                                  False, True, None, False, None)

get_lower_buffer = configForMethod("portfolio",
                                   "get_actual_buffers_for_position",
                                   "Lower buffer",
                                   False, True, "bot_pos", False, None)

get_upper_buffer = configForMethod("portfolio",
                                   "get_actual_buffers_for_position",
                                   "Upper buffer",
                                   False, True, "top_pos", False, None)


def get_stage_breakdown_over_codes(data_backtest,
                                   method_list = []):

    value_dict =  {}
    for method_config in method_list:
        value_dict[method_config.name] = get_list_of_values_by_instrument_for_config(data_backtest, method_config)
...
def get_list_of_values_by_instrument_for_config(data_backtest, config_for_method):
    instrument_codes = data_backtest.system.get_instrument_list()
    datetime_cutoff = from_marker_to_datetime(data_backtest.timestamp)
    ...
    if config_for_method.requires_code_bool:
        ## call for each code
        if config_for_method.col_selector is not None:
            value_list = [method(instrument_code).ffill()[:datetime_cutoff].iloc[-1][config_for_method.col_selector]
                          for instrument_code in instrument_codes]
        else:
            value_list = [method(instrument_code).ffill()[:datetime_cutoff].iloc[-1]
                          for instrument_code in instrument_codes]
...
def get_current_position_df_for_instrument_code_list(data_backtest, data, instrument_code_list):
    position_list = [get_current_position_for_instrument_code(data_backtest, data, instrument_code)
                     for instrument_code in instrument_code_list]
    position_df = pd.DataFrame(position_list, index = instrument_code_list, columns=['Current position'])
    return position_df

def get_position_at_timestamp_df_for_instrument_code_list(data_backtest, data, instrument_code_list):
    position_list = [get_position_for_instrument_code_at_timestamp(data_backtest, data, instrument_code)
                     for instrument_code in instrument_code_list]
    position_df = pd.DataFrame(position_list, index = instrument_code_list, columns=['Position at timestamp'])
    return position_df


def get_position_for_instrument_code_at_timestamp(data_backtest, data, instrument_code):
    diag_positions = diagPositions(data)
    positions_over_time = diag_positions.get_position_df_for_strategy_and_instrument(data_backtest.strategy_name, instrument_code)
    datetime_cutoff = from_marker_to_datetime(data_backtest.timestamp)
