...
def format_roll_data_for_instrument(results_dict):
    ...
    formatted_output.append(header("Roll status report produced on %s" % str(datetime.datetime.now())))
    ...
    # sort by time to theoretical roll, and apply same sort order for all tables
    ...
    formatted_output.append(body_text("Roll_exp is days until preferred roll set by roll parameters. Prc_exp is days until price contract rolls, Crry_exp is days until carry contract rolls"))
    ...
    table3_dict={}
    for col_number in range(width_contract_columns):
         table3_dict["V%d" % col_number] = [results_dict[code]['volumes'][col_number]
                                        for code in instrument_codes]
    ...
    formatted_output.append(body_text("Contract volumes over recent days, normalised so largest volume is 1.0"))

    table4_dict={}
    for col_number in range(width_contract_columns):
         table4_dict["P%d" % col_number] = [results_dict[code]['curve'][col_number]
                                        for code in instrument_codes]
