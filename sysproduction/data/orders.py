from syscore.objects import arg_not_supplied, missing_data, success, failure, no_parent, missing_order
...
class dataOrders(object):
    ...
    def get_historic_broker_orders_in_date_range(self, period_start, period_end=arg_not_supplied):
        # remove split orders
        order_id_list = self.data.db_broker_historic_orders.get_orders_in_date_range(period_start, period_end=period_end)
        order_id_list = [order_id for order_id in order_id_list
    ...
    def get_historic_broker_order_from_order_id_with_execution_data(self, order_id):
        order = self.get_historic_broker_order_from_order_id(order_id)
        reference_price = self.get_reference_price_for_historic_broker_order_id(order_id)
        generated_datetime = self.get_generated_datetime_for_historic_broker_order_id(order_id)
        parent_limit = self.get_parent_limit_for_historic_broker_order_id(order_id)
        ...
        if order.is_split_order:
            ## We won't use these, and it may cause bugs for orders saved with legacy data
            calc_mid = None
            calc_side = None
            calc_fill =None
        else:
    ...
    def get_generated_datetime_for_historic_broker_order_id(self, order_id):
        instrument_order = self.get_parent_instrument_order_for_historic_broker_order_id(order_id)
    ...
    def get_parent_contract_order_for_historic_broker_order_id(self, order_id):
        ...
        contract_order = self.get_historic_contract_order_from_order_id(contract_order_id)

        return contract_order

    def get_parent_instrument_order_for_historic_broker_order_id(self, order_id):
        contract_order = self.get_parent_contract_order_for_historic_broker_order_id(order_id)
        ...
        instrument_order = self.get_historic_instrument_order_from_order_id(instrument_order_id)
