...
def get_data_and_create_csv_directories(logname):
    ...
    data = dataBlob(csv_data_paths=class_paths, keep_original_prefix=True, log_name=logname)

    data.add_class_list("csvFuturesContractPriceData csvFuturesAdjustedPricesData \
...
def backup_contract_position_data(data):
    instrument_list = data.mongo_contract_position.get_list_of_instruments_with_any_position()
    for instrument_code in instrument_list:
        contract_list = data.mongo_contract_position.get_list_of_contracts_with_any_position_for_instrument(instrument_code)
        for contract in contract_list:
            mongo_data = data.mongo_contract_position.\
                get_position_as_df_for_instrument_and_contract_date(instrument_code, contract)
            data.csv_contract_position.\
                write_position_df_for_instrument_and_contract_date(instrument_code, contract,
                                                                   mongo_data)
            data.log.msg("Backed up %s %s contract position data" % (instrument_code, contract))
...
def backup_strategy_position_data(data):
    ...
    for strategy_name in strategy_list:
        for instrument_code in instrument_list:
            mongo_data = data.mongo_strategy_position.\
                get_position_as_df_for_strategy_and_instrument(strategy_name, instrument_code)
            if mongo_data is missing_data:
                continue
            data.csv_strategy_position.\
                write_position_df_for_instrument_strategy(strategy_name, instrument_code,
                                                                   mongo_data)
            data.log.msg("Backed up %s %s strategy position data" % (instrument_code, strategy_name))


def backup_historical_orders(data):
    data.log.msg("Backing up strategy orders...")
    list_of_orders = [data.mongo_strategy_historic_orders.get_order_with_orderid(id) for id in data.mongo_strategy_historic_orders.get_list_of_order_ids()]
    ...
    list_of_orders = [data.mongo_broker_historic_orders.get_order_with_orderid(id) for id in data.mongo_broker_historic_orders.get_list_of_order_ids()]
...
def backup_capital(data):
    ...
    capital_data = dict()
    for strategy_name in strategy_list:
        capital_data[strategy_name] = data.mongo_capital.get_capital_pd_series_for_strategy(strategy_name)

    capital_data['TOTAL_total'] = data.mongo_capital.get_total_capital_pd_series()
    capital_data['TOTAL_broker'] = data.mongo_capital.get_broker_account_value_pd_series()
    capital_data['TOTAL_max'] = data.mongo_capital.get_maximum_account_value_pd_series()
    capital_data['TOTAL_pandl'] = data.mongo_capital.get_profit_and_loss_account_pd_series()

    capital_data = pd.concat(capital_data, axis=1)
    capital_data.columns = strategy_list+['TOTAL_total', 'TOTAL_broker', 'TOTAL_max', 'TOTAL_pandl']
...
def backup_optimal_positions(data):
    ...
    for strategy_name in strategy_list:
        instrument_list = data.mongo_optimal_position.get_list_of_instruments_for_strategy_with_optimal_position(
            strategy_name)
        for instrument_code in instrument_list:
            mongo_data = data.mongo_optimal_position.\
                get_optimal_position_as_df_for_strategy_and_instrument(strategy_name, instrument_code)
            if mongo_data is missing_data:
                continue
            data.csv_optimal_position.\
                write_position_df_for_instrument_strategy(strategy_name, instrument_code,
                                                                   mongo_data)
            data.log.msg("Backed up %s %s optimal position data" % (instrument_code, strategy_name))
...
def backup_contract_data(data):
    ...
    for instrument_code in instrument_list:
        contract_list = data.mongo_futures_contract.get_all_contract_objects_for_instrument_code(instrument_code)
        data.csv_futures_contract.write_contract_list_as_df(instrument_code, contract_list)
